...
# SparkSession must be created inside the container where the job runs, not at the module level where it would run during Airflow's DAG parsing. We'll create it inside main() instead.
...
def process_leaderboards(data: dict) -> dict:
    ...
    for catergory, players in data.items():
        try:
            ...
            spark_dfs[catergory] = spark.createDataFrame(pdf).orderBy("rank")
            log.info(f"Processed {catergory} — {len(players)} players")
        except Exception as e:
            log.error(f"Failed to process {catergory}: {e}")
